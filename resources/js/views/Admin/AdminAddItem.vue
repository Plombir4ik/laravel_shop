<template>
    <div class="container mt-4 add-container">
        <div class="col-md-6 col-10 mx-auto">
            ...
            <form
                @submit.prevent="addItem($route.params.cat)"
                class="mb-5"
            >
                ...
                <div v-for="spec in oldSpecs" :key="spec.id">
                    <div class="form-group">
                        ...
                        <b-form-input @update="updateSpec(spec.id, $event)" type="text" class="form-control" :name="'spec_' + spec.id"/>
                    </div>
                </div>
...
export default {
    ...
    async created() {
        try {
            ...
            res.data.forEach(
                (spec) =>
                    this.newSpecs.push({id: spec.id, value: ""})
            )
        } catch (e) {
            console.error(e);
        }
    ...
    methods: {
        ...
        addItem(cat_id) {
            axios.post("/api/products/" + this.$route.params.cat, {
