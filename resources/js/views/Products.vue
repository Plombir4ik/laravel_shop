            ...
            <b-row>
                <b-column v-if="display" class="col-md-3 col-sm-12 mt-2 filters">
                    <div v-for="spec of specs" :key="spec.name">
                        ...
                        <div v-for="value of spec.values" :key="value">
                            <label>
                                <input
                                    type="checkbox"
                                    @change="newFilter(spec.id, value, $event)"
                                />
                                {{ value }}
                            </label>
                        </div>
...
export default {
    ...
    methods: {
        ...
        async newFilter(id, value, event) {
            const index = this.checked.names.indexOf(id);
            if (event.target.checked) {
                if (index !== -1) {
                    this.checked.values[index] += `,${value}`;

                } else{
                    this.checked.names.push(id);
                    this.checked.values.push(value);
                }
            } else {
                let tempArr = this.checked.values[index].split(",");
                if (tempArr.length > 1) {
                    let subIndex = tempArr.indexOf(value);
                    tempArr.splice(subIndex, 1);
                    this.checked.values[index] = tempArr.toString();
                } else {
                    this.checked.names.splice(index, 1);
                    this.checked.values.splice(index, 1);
                }

            }

            // Формируем строку параметров фильтрации для добавления к URL
            let params = '';
            if (this.checked.names.length > 0) {
                params = this.checked.names.map((name, index) => `${name}=${this.checked.values[index]}`).join('&');
            }

            // Формируем URL-адрес с учетом параметров фильтрации
            const url = params !== '' ? `/api/categories/${this.$route.params.category_id}/${this.$route.params.subcategory_id}?${params}` : `/api/categories/${this.$route.params.category_id}/${this.$route.params.subcategory_id}`;

            // Выполняем запрос данных с учетом фильтров
            try {
                const res = await axios.get(url);
                // Обновляем products, чтобы Vue.js мог реагировать на изменения
                this.products = res.data;
                if (this.products.length != 0) {
                    this.displayProducts = 3;
                } else {
