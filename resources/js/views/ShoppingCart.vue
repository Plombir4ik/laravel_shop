                ...
                <div class="col col-lg-7 col-12">
                    ...
                    >
                        <div class="row">
                            ...
                            <div role="button" class="col my-auto col-lg-2 col-1">
                                <a  @click="deleteFromCart(item.id)">
                                    <svg
                    ...
                    <form class="ordering" @submit.prevent="addOrder()">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <p class="user-info">
                                    Ім'я та прізвище: {{ getUser.name }} {{ getUser.surname }}
                                </p>
                                <p class="user-info">Електронна пошта: {{ getUser.email }}</p>
                                <p class="user-info">Номер телефону: {{ getUser.phone }}</p>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                ...
                                <b-form-select
                                    ...
                                    v-model="key_payment"
                                >
                                    <b-form-select-option disabled class="disabled-option" value="1"
                                    >Картою онлайн
                                    </b-form-select-option
                                    >
                                    <b-form-select-option default value="2"
...
export default {
    data() {
        return {
            ...
            currentValue: 0,
            countDict: {},
            apiKey: process.env.MIX_NOVA_POSHTA_KEY,
            ...
            axios
                ...
                .then((response) => {
                    ...
                    for (var i = 0; i < this.cartItems.length; i++) {
                        this.fullPrice += this.cartItems[i].price * this.cartItems[i].user_count;
                    }
        ...
        changeCount(event, id) {
            ...
            for (var i = 0; i < this.cartItems.length; i++) {
                if (!(this.cartItems[i].id in this.countDict)) {
                    this.countDict[this.cartItems[i].id] = this.cartItems[i].user_count;
                }
    ...
    computed: {
      getUser(){
          return this.$parent.$refs.header.user;
      }
    },
    mounted() {
        ...
        axios
            ...
            .then((response) => {
                ...
                for (var i = 0; i < this.cartItems.length; i++) {
                    if(this.cartItems[i].count == 0) this.deleteFromCart(this.cartItems[i].id)
                    if(this.cartItems[i].user_count > this.cartItems[i].count){
                        this.cartItems[i].user_count = this.cartItems[i].count
                        this.changeCount(this.cartItems[i].count, this.cartItems[i].id);
                    }
                    this.fullPrice += this.cartItems[i].price * this.cartItems[i].user_count;
                }
                console.log(this.cartItems);
...
.disabled-option {
    background-color: #E3E3E3; /* Серый цвет фона */
}
